...
namespace Net.Teirlinck.Utils.WPF
{
    public class ObservableConcurrentDictionary<TKey, TValue> : ConcurrentDictionary<TKey, TValue>, INotifyCollectionChanged, INotifyPropertyChanged
    {
        ...
        public new TValue AddOrUpdate(TKey key, Func<TKey, TValue> addValueFactory, Func<TKey, TValue, TValue> updateValueFactory)
        {
            ...
            (k, oldValue) =>
            {
                // 1. Check if TValue implements INotifyPropertyChanged and hookup PropertyChanged event
                ...
                // 3. Notify of the collection changed
                // Do note raise any CollectionChanged for action 'Replace' here: this is an update and not a replace. The update will be handled by the item's NotifyPropertyChanged
                // Raising a CollectionChange in this situation will result in an IndexOutOfBond exception
                //CollectionChanged?.Invoke(this, new NotifyCollectionChangedEventArgs(Replace, updatedValue, oldValue));
